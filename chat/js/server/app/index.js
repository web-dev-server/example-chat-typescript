...
App.prototype = {
        ...
        if (eventName == 'login') {
            ...
            if (recepient == 'all') {
                ...
                if (typeof (this.onlineUsers[recepient]) != 'undefined')
                    this.sendToSingle('message', data, this.onlineUsers[recepient].sessionId);
            ...
            if (recepient == 'all') {
                ...
                if (typeof (this.onlineUsers[recepient]) != 'undefined')
                    this.sendToSingle('typing', this.typingUsers, this.onlineUsers[recepient].sessionId);
            }
    ...
    loadCsvLoginData: function (cb) {
        fs.readFile(__dirname + '/../../../data/login-data.csv', function (err, content) {
    ...
    sendLastComunication: function (ws, sessionId) {
        ...
        if (this.data.length > 0) {
            var lastMessagesCount = App.LAST_CHAT_MESSAGES_TO_SEND, response = {};
            for (var i = Math.max(this.data.length - 1 - lastMessagesCount, 0), l = Math.min(lastMessagesCount, this.data.length); i < l; i += 1) {
                response = this.data[i];
                if (response.eventName !== 'message')
                    continue;
                if (response.data.recepient != 'all' &&
